    ...
    bindEvents:function(){
      ...
      function listenToUser(view){
        ...
        if(document.body.ontouchstart !== undefined){
          //触屏设备
          console.log('触屏设备')
          view.addEventListener('touchstart',(a)=>{
          ...
          view.addEventListener('touchmove',(a)=>{
            let x = (a.touches[0].clientX)
            let y = (a.touches[0].clientY)
            if(!using){ return }
            if(eraserEnabled){
          ...
          //非触屏设备
          //按下鼠标
          view.addEventListener('mousedown',(a)=>{
            let x = (a.clientX + 14)
            let y = (a.clientY + 29)
            using = true
            if(eraserEnabled){
      ...
      //动鼠标
          view.addEventListener('mousemove',(a)=>{
            let x = (a.clientX + 14)
            let y = (a.clientY + 29)
            if(!using){ return }
            if(eraserEnabled){
      ...
      //松开鼠标
          view.addEventListener('mouseup',(a)=>{
            using = false
          })
        }
      }
        ...
        function model(view){
          window.onload = function(){
            ...
            eraser.addEventListener('click',()=>{
              ...
              view.style.cursor = 'url(./images/eraser.ico),auto'
            })
            pen.addEventListener('click',()=>{
              eraserEnabled = false
              pen.classList.add('active')
              eraser.classList.remove('active')
              view.style.cursor = 'url(./images/pen.ico),auto'
            })
          }
        }
        //颜色选择
        ...
        download.addEventListener('click',()=>{
          let url = view.toDataURL("image/png")
